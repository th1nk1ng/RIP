...
BOOL CARPLayer::Send(unsigned char* ppayload, int nlength,int dev_num){
	...
	if((index = SearchIpAtTable(routerDlg->m_IPLayer->GetDstIP())) != -1){ //serch 결과 존재할 경우
...
BOOL CARPLayer::Receive(unsigned char* ppayload,int dev_num){
	...
	if(receive_arp_message->arp_op == request){ //요구
		if(memcmp(receive_arp_message->arp_destprotoaddr,routerDlg->m_IPLayer->GetSrcIP(dev_num),4)){// 내 ip가 아닌대 올경우
...
int CARPLayer::SearchIpAtTable(unsigned char Ip_addr[4]){ //ip찾는 함수
	int count;
	int i;
	int ret = -1;
	CACHE_ENTRY temp;
	if(Cache_Table.GetCount() == 0) {		//Cache table 에 아무것도 없을 경우
		return -1;
	} else {								//존재 할 경우
